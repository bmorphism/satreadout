...
$\mathrm{satDist}_a$. The bridge to human vision is \cref{hyp:bujack} alone — an
empirical premise (which discrimination data may refine or refute), never a
theorem. Thus the precise formalized statement is: \emph{if} perceptual difference
is a saturating readout (\cref{hyp:bujack}), \emph{then} perceptual color space is
provably non-Riemannian (\cref{thm:break}). The Bujack et al.\ PNAS 2022 result is
the evidence for the antecedent; \cref{thm:break} is the (now machine-checked)
consequent.
