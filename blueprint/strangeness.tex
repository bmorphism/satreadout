...
\item \textbf{Classical, now reduced to one named theorem.} \cref{thm:notgeo}
machine-checks that $\mathrm{satDist}_a$ is \emph{not geodesic}. The only step from
there to the word ``Riemannian'' is \emph{Hopf--Rinow} (complete Riemannian $\Rightarrow$
geodesic) — a precise, famous theorem, not hand-waving. Formalizing a general
geodesic/length-space typeclass and Hopf--Rinow would erase even this.
\end{itemize}
Precise formalized statement: \emph{if} perceptual difference is a saturating
readout (\cref{hyp:bujack}), \emph{then} perceptual color space is a non-geodesic
metric (\cref{thm:notgeo}, machine-checked), hence — by Hopf--Rinow — non-Riemannian.
